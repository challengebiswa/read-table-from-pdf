...
def fil_approx_boxes(img):
    ...
    cv2.imwrite("output/test.jpeg", img)
...
def extract_text(image_name, img_path="temp/final_masked.jpeg"):
    ...
    idx = 1
    for cnt in contours:
        file_name = "output/"+image_name+"_"+str(idx)
        x, y, w, h = cv2.boundingRect(cnt)
        ...
        roi = cv2.resize(roi, (int(w*1.25), int(h*1.25)),
...
def predict_table_masks(img):
    ...
    cv2.imwrite("temp/pred_col.jpeg", pred_col)
    cv2.imwrite("temp/pred_table.jpeg", pred_table)
